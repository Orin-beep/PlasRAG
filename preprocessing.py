...
parser.add_argument('--fasta',
                    help='FASTA file of the input plasmid DNA sequences (either complete sequences or contigs) to be characterized or retrieved by the PlasRAG tool, default: example_data/test_plasmids.fasta',
                    default='example_data/test_plasmids.fasta')
parser.add_argument('--model_path', help='path of the folder storing the downloaded models, default: models', type=str,
                    default='models')
parser.add_argument('--esm',
                    help="Path of the ESM-2 model (esm2_t33_650M_UR50D.pt), which can be downloaded with the 'download_esm.py' script, default: esm_models/esm2_t33_650M_UR50D.pt",
                    default='esm_models/esm2_t33_650M_UR50D.pt')
parser.add_argument('--midfolder', help='folder to store the intermediate files for prediction, default: temp',
...
raw_plasmids = list(SeqIO.parse(fasta_path, 'fasta'))

# Split FASTA into parts for parallel Prodigal
split_dir = os.path.join(out_fn, 'split_fasta')
os.makedirs(split_dir, exist_ok=True)
num_jobs = min(inputs.threads, len(raw_plasmids))
split_size = (len(raw_plasmids) + num_jobs - 1) // num_jobs

fasta_splits = []
for i in range(0, len(raw_plasmids), split_size):
    part = raw_plasmids[i:i + split_size]
    part_path = os.path.join(split_dir, f"part_{i // split_size}.fasta")
    SeqIO.write(part, part_path, 'fasta')
    fasta_splits.append(part_path)


# Function for parallel Prodigal execution
def run_prodigal(inp):
    part, idx = inp
    faa_out = os.path.join(out_fn, f"plasmids_{idx}.faa")
    cmd = f"prodigal -i {part} -a {faa_out} -f gff -p meta"
    subprocess.check_call(cmd, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    return faa_out


# Run Prodigal in parallel using multiprocessing Pool
print("Running Prodigal in parallel ...")
with Pool(num_jobs) as pool:
    faa_parts = pool.map(run_prodigal, [(path, i) for i, path in enumerate(fasta_splits)])

# Merge all partial FAA files into one

merged_faa = os.path.join(out_fn, "plasmids.faa")
with open(merged_faa, 'w') as out_f:
    for p in faa_parts:
        with open(p, 'r') as infile:
            out_f.write(infile.read())

# Process translated proteins
input_ids = defaultdict(list)
protein_idx = []
idx = 1
proteins = []
for s in SeqIO.parse(merged_faa, 'fasta'):
...
SeqIO.write(proteins, merged_faa, 'fasta')

# Check and report discarded plasmids (too short for translation)
discarded = set(r.id for r in raw_plasmids) - set(input_ids)
if (len(discarded) > 1):
    discarded_str = ', '.join(list(discarded))
    print(
        f'{len(discarded)} plasmids have been discarded because they are too short for gene translation: {discarded_str}.')
elif (len(discarded) == 1):
    discarded_str = ', '.join(list(discarded))
    print(f'1 plasmid has been discarded because it is too short for gene translation: {discarded_str}.')
...
toks_per_batch = 4096
dataset = FastaBatchedDataset.from_file(merged_faa)
batches = dataset.get_batch_indices(toks_per_batch, extra_toks_per_seq=1)
data_loader = torch.utils.data.DataLoader(
    dataset, collate_fn=alphabet.get_batch_converter(truncation_seq_length), batch_sampler=batches,
    num_workers=inputs.threads)  # Use multi-workers for loading
...
esm_dict = {}
with torch.no_grad():
    for batch_idx, (labels, strs, toks) in enumerate(tqdm(data_loader, desc="Computing ESM embeddings")):
...
f = open(os.path.join(out_fn, "plasmids.csv"), 'w')
...
# preprocess dataset
dataset = dataset.map(preprocess_data, batched=True, num_proc=inputs.threads,
                      remove_columns=dataset['test'].column_names, keep_in_memory=True)
...
vocab_size_dict = {'amr': 124, 'host': 199, 'eco': 58, 'ecohost': 43, 'vf': 205, 'risk': 6, 'mob': 41, 'inc': 59,
                   'metal': 231, 'char': 15}
...
for domain in ['amr', 'vf', 'host', 'ecohost', 'eco', 'char', 'metal', 'inc', 'risk', 'mob']:
    ...
    test_args = TrainingArguments(
        ...
        dataloader_num_workers=inputs.threads  # Add multi-workers for data loading
    )
