...
parser.add_argument('--query',
                    help="question or instruction regarding the query plasmids, default: 'Please summarize key information from the most relevant literature.'",
                    type=str, default='Please summarize key information from the most relevant literature.')
parser.add_argument('--out', help='path to store the prediction results, default: results', type=str, default='results')
parser.add_argument('--llm',
                    help=" whether to enable LLM for result summarization and question answering, default: 'False'",
                    type=str, default="False")
parser.add_argument('--llm_backend', help="choose llm backend: 'local' or 'api'", type=str, default=None)
parser.add_argument('--model', help=" the downloaded model id, default: 'Qwen/Qwen3-30B-A3B-Instruct-2507'",
                    type=str, default="~/.cache/huggingface/hub/models--Qwen--Qwen3-30B-A3B-Instruct-2507/snapshots/0d7cf23991f47feeb3a57ecb4c9cee8ea4a17bfe/")
parser.add_argument('--midfolder', help='the intermediate folder generated by preprocessing.py, default: ./temp',
...
def help_info():
    ...
    print("""Usage of characterize.py:
        [--query QUERY] question or instruction regarding the query plasmids
        [--out OUT] path to store prediction results
        [--llm LLM] whether to enable LLM
        [--llm_backend BACKEND] choose 'local' or 'api'
        [--model MODEL] the model name or API model ID
        [--midfolder MIDFOLDER] temp folder
        [--database DATABASE] path to DB
...
def assemble(L, domain):
    if (domain == 'vf'):
        ...
        if not x:
            sentence += f'No topology output and {com} for this'
        else:
            top = get_large(x)
            top = top[:top.rfind(' ')]
...
domains = {'eco': 'Ecosystem', 'vf': 'Virulence Factor', 'metal': 'Metal Resistance', 'inc': 'Incompatibility Group',
           'risk': "Risk Index", 'host': 'Host Range', 'mob': 'Mobility', 'char': 'Basic Properties', 'amr': 'AMR'}
for pls in res_dict:
    tsv_w = csv.writer(open(f'{res_fn}/{pls}.csv', 'w'), delimiter=',')
...
def generate_prompt(question, ref, abstract, description_dict):
    ...
    **Instructions**:
    ...
    4. When analyzing the literature, summarize key information about the reference {ref} based on the abstract.
    '''
...
if backend is None:
    print("Error! please input the llm backend parameter!")
    exit(0)

llm = inputs.model
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

print("test machine:",device)

# ==========================================================
#  OPTION 1: Local backend (HuggingFace model)
# ==========================================================
if backend == "local":
    ...
    for pls in prompt_dict:
        tsv_w = csv.writer(open(f'{res_fn}/{pls}_ans.csv', 'w'), delimiter=',')
        tsv_w.writerow(['Item', 'Content'])
...
# ==========================================================
#  OPTION 2: API backend (OpenAI-compatible)
# ==========================================================
elif backend == "api":
    ...
    print("Environment variables (api key and url) loaded successfully!")
    ...
    raise ValueError(f"Unknown backend mode '{backend}'. Choose 'local' or 'api'.")
